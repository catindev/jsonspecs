...
Старый пакет `jsonspecs` остаётся совместимым переходным пакетом и экспортирует тот же публичный API.
...
`@jsonspecs/rules` не привязан к загрузчику. Чтение файлов, project manifest, Studio UI и сборка snapshot находятся в [`jsonspecs-cli`](https://www.npmjs.com/package/jsonspecs-cli), а не в ядре.
